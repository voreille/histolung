...
from histolung.legacy.utils import yaml_load
...
def main(wsi_name, sigma):
    ...
    # sample = select_wsi[0]
    print(
        f"Heatmap for sample: {wsi_name} with Groundtruth label: {groundtruth}"
    )
    ...
    metadata_preds = pd.read_csv(sampledir / f"{wsi_name}_coords_densely.csv",
                                 header=None)
